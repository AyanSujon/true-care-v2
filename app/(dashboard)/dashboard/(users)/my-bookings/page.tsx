...
// import { authOptions } from "@/lib/authOption"; // same as used in your server action
...
//   // If no session at all, show not logged in (or redirect if you prefer)
...
//   // First, try to find user in DB to get their _id (for logged-in users)
//   const userCollection = await dbConnect(collections.USERS);
//   const loggedInUser = await userCollection.findOne({ email: session.user.email });

//   if (loggedInUser?._id) {
//     // Logged-in user: fetch by userId (includes their own + any guest bookings linked later)
//     bookings = await bookingCollection
//       .find({ userId: loggedInUser._id })
//       .sort({ createdAt: -1 }) // newest first
//       .toArray();
//   } else {
//     // Unlikely if properly authenticated, but fallback to email (guest bookings)
//     bookings = await bookingCollection
